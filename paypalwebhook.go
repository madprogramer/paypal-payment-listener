...
//
// Returns status, account name, amount and an error if it occurs.
func PaypalWebhook(c *paypal.Client, w http.ResponseWriter, r *http.Request,
	balance_update_function func(params ...interface{}),
	params ...interface{}){
	...
	if r.Method == "POST" {
		...
		if event_type == "CHECKOUT.ORDER.APPROVED" {
		    ...
		    if v {
				...
				// _, err = db.Exec(`insert into TABLE values(?,?,compress(?),?,?)`, 
		  //                                 account, amount, Now, "Y")
				// if err != nil { log.Fatal("Payments error ", err, ""); return  }
